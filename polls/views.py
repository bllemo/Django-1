...
class IndexView(generic.ListView):
    ...
    def get_queryset(self):
        if self.request.user.is_staff or self.request.user.is_superuser:
            ...
            print(datetime.now())
            object_list = Question.objects.order_by('-pub_date').filter(death_date__gte=timezone.now())
            return object_list

# вместо class DetailView лучше использовать отдельную функцию, т.к. это нужно для успешного перенаправления и
# гибкости
def question_detail_view(request, pk):
    question = get_object_or_404(Question, pk=pk)
    if UserChoice.objects.filter(user=request.user.pk, question=pk, ).exists():
        return render(request, 'polls/question/results.html', {'question': question, 'error_message': 'Вы уже сделали выбор'})
...
def vote(request, question_id):
    ...
    # проверить если пользователь пытается проголосовать хитрым способом
    if UserChoice.objects.filter(user=request.user.pk, question=question_id, ).exists():
        return render(request, 'polls/question/results.html', {
            'question': question,
    ...
    try:
        ...
        # изменение количества проголосовавших
        ...
        choice.votes += 1
        choice.save()
    # if you get more than one answer (debug)
